...
class MealPlanMode(Mode): 
        ...
        if (mode.breakFastWindowOpen or mode.lunchWindowOpen or mode.dinnerWindowOpen): 
            if ( 80<event.y<170): 
                ...
                if (mode.breakFastWindowOpen): 
                    ...
                    mode.dinnerWindowOpen = False 
                    mode.currentMealPlans = []
            mode.app.suggestedFoods = mode.breakfastPlan + mode.lunchPlan + mode.dinnerPlan
    ...
    # mealPlan should be mode.breakfastPlan...
    def calorieForMealPlan(mode, mealPlan): 
        if (mealPlan != []): 
            calorie = 0 
            for v in mealPlan: 
                values = mode.app.refreshedD[v]
                calorie += (int)(values[0])
            return calorie
        else: 
            return '**'
    ...
    def drawBreakFastBoard(mode, canvas): 
        ...
        totalCalorie = mode.calorieForMealPlan(mode.breakfastPlan)
        canvas.create_text(475, 125, text=f'{totalCalorie}kcal >', anchor='e', fill='grey', font='Chalkduster 20 bold')
        r = 5.5
        if (mode.breakfastPlan == []): 
            color = rgbString(175, 175, 175)
            fColor = 'white'
        else: 
            color = 'grey'
            fColor = rgbString(230, 230, 230)
        canvas.create_oval(347-r, 125-r, 347+r, 125+r, fill=f'{fColor}', outline=f'{color}', width=5)
        # draw mealPlan 
        if (mode.breakfastPlan != []): 
            for i in range(3): 
                values = mode.app.refreshedD[mode.breakfastPlan[i]]
                canvas.create_text(50, 180+i*50, text=f'· {mode.breakfastPlan[i]}: {values[0]} kcal, {values[1]}g, {values[2]}g, {values[3]}g', anchor='sw', fill='grey', font='Calibri 10')
    ...
    def drawLunchBoard(mode, canvas): 
        ...
        totalCalorie = mode.calorieForMealPlan(mode.lunchPlan)
        canvas.create_text(475, 350, text=f'{totalCalorie}kcal >', anchor='e', fill='grey', font='Chalkduster 20 bold')
        r = 5.5
        if (mode.lunchPlan == []): 
            color = rgbString(175, 175, 175)
            fColor = 'white'
            ...
            color = 'grey'
            fColor = rgbString(230, 230, 230)
        canvas.create_oval(310-r, 350-r, 310+r, 350+r, fill=f'{fColor}', outline=f'{color}', width=5)
        if (mode.lunchPlan != []): 
            for i in range(3): 
                values = mode.app.refreshedD[mode.lunchPlan[i]]
                canvas.create_text(50, 405+i*50, text=f'· {mode.lunchPlan[i]}: {values[0]} kcal, {values[1]}g, {values[2]}g, {values[3]}g', anchor='sw', fill='grey')
    ...
    def drawDinnerBoard(mode, canvas): 
        ...
        totalCalorie = mode.calorieForMealPlan(mode.dinnerPlan)
        canvas.create_text(475, 575, text=f'{totalCalorie}kcal >', anchor='e', fill='grey', font='Chalkduster 20 bold')
        r = 5.5
        if (mode.lunchPlan == []): 
            color = rgbString(175, 175, 175)
            fColor = 'white'
            ...
            color = 'grey'
            fColor = rgbString(230, 230, 230)
        canvas.create_oval(314-r, 575-r, 314+r, 575+r, fill=f'{fColor}', outline=f'{color}', width=5)
        if (mode.dinnerPlan != []): 
            for i in range(3): 
                values = mode.app.refreshedD[mode.dinnerPlan[i]]
                canvas.create_text(50, 630+i*50, text=f'· {mode.dinnerPlan[i]}: {values[0]} kcal, {values[1]}g, {values[2]}g, {values[3]}g', anchor='sw', fill='grey')
...
class HomeMode(Mode): 
    ...
    def mousePressed(mode, event): 
        ...
        if (mode.metaWindowOpen): 
            if (200 < event.x < 220): 
                if (110 < event.y < 120): 
                    ...
                    mode.color[0] = 'grey'
                    mode.metaWindowOpen = False 
                    #if (type(mode.app.totalCalorieNeed, str)): 
        ...
        # goto foodPage
        if (25 < event.x < 475) and (440 < event.y < 475): 
            mode.app.setActiveMode(mode.app.foodPage)
    ...
    def drawInfoBoard(mode, canvas): 
        ...
        canvas.create_text(45, 100, text=f'· {mode.app.name}', font='Chalkduster 25 bold', fill='grey', anchor='sw')
    ...
    def drawFoodDisplayBoard(mode, canvas): 
        ...
        canvas.create_text(45, 285, anchor='nw', text='· Food Mode', font='Chalkduster 22 bold', fill='grey')
        canvas.create_text(mode.width/2, 440, fill=f'{rgbString(175, 175, 175)}', text='--------------------------', font='Chalkduster 25')
        canvas.create_text(mode.width/2, 455, text='See More ->', font='Chalkduster 25', fill=f'{rgbString(175, 175, 175)}')
        # draw foodPercentage bar
        names = ['B', 'L', 'D', 'E']
        width = 15
        space = 35
        height = 90 
        startX, startY = 340, 310
        calorieNeedForEachMeal = [mode.app.totalCalorieNeed*0.35, mode.app.totalCalorieNeed*0.4, mode.app.totalCalorieNeed*0.3]
        if (mode.app.totalCalorieNeed != 0): 
            intakePercent = [(mode.app.breakFastIntakeCalorie/calorieNeedForEachMeal[0]), (mode.app.lunchIntakeCalorie/calorieNeedForEachMeal[1]), (mode.app.dinnerIntakeCalorie/calorieNeedForEachMeal[2]) ]
        else: 
            intakePercent = [0, 0, 0]

        for i in range(3): 
            canvas.create_rectangle(startX + i*space, startY, startX+i*space + width, startY+height, fill='white', outline=f'{rgbString(175, 175, 175)}', width=4)
            canvas.create_text(startX + i*space + width/2, startY+height+15, text=f'{names[i]}', fill=f'{rgbString(175, 175, 175)}', font='Chalkduster 18 bold' )
            # draw percent bar 
            if (intakePercent[i] > 1): 
                percent = 1
            else: 
                percent = intakePercent[i]
            canvas.create_rectangle(startX + i*space, startY+height-height*percent, startX+i*space + width, startY+height, fill=f'{rgbString(175, 175, 175)}', outline=f'{rgbString(175, 175, 175)}', width=4)
            # draw perentage 
            canvas.create_text(startX+i*space+width/2, startY-10, text=f'{round(intakePercent[i]*100, 1)}%', fill=f'{rgbString(175, 175, 175)}', font='Chalkduster 10' )

        # draw info 
        total = mode.app.breakFastIntakeCalorie + mode.app.lunchIntakeCalorie + mode.app.extraIntakeCalorie + mode.app.extraIntakeCalorie
        if (mode.app.totalCalorieNeed != 0): 
            left = mode.app.totalCalorieNeed - total
        else: 
            left = 0
        canvas.create_text(70, 340, anchor='nw', text=f'· Calories Need: {mode.app.totalCalorieNeed} kcal', font='Chalkduster 15', fill='grey')
        canvas.create_text(70, 380, anchor='nw', text=f'· Still Need: {left} kcal', font='Chalkduster 15', fill='grey')
    ...
    def drawMealPlanBoard(mode, canvas): 
        ...
        canvas.create_text(45, 535, anchor='nw', text='· Suggested Meal Plan', font='Chalkduster 22 bold', fill='grey')
        ...
        if (len(mode.app.suggestedFoods) < 8): 
            foods = mode.app.suggestedFoods
        else: 
            foods = mode.app.suggestedFoods[:8]
        startY = 590
        for i in range(len(foods)): 
            canvas.create_text(70, startY+i*20, text=f'· {foods[i]}', font='Chalkduster 10', fill='grey', anchor='nw')
...
class FoodMode(Mode): 
    ...
    def searchData(mode, calorieUrl): 
        ...
        mode.intakeCalorie = 0 
        mode.leftCalorie = mode.app.totalCalorieNeed
    ...
    def getTotalCalorie(mode): 
        ...
        if (foods != []): 
            total = 0 
            for i in range(len(foods)): 
                calorie = (int)(mode.directoryDict[foods[i]][0])
                total += calorie
            return total 
        else: 
            return 0
    ...
    def mousePressed(mode, event): 
        ...
        if (mode.searchBoardOpened): 
            ...
            # Exit search board 
                ...
                (mode.currentMeal, mode.currentMealQuant) = (None, None)
                mode.addToSavedData(which, mode.currentMeal, mode.currentMealQuant)
                ...
                mode.updateIntakeCaloriesEach()

            
    # currentMeal: 0-b, 1-l, 2-d, 3-e
    def addToSavedData(mode, currentMeal, foodList, quantList): 
        pass            
    ...
    def drawDashboard(mode, canvas): 
        ...
        rOut, rIns = 40, 30
        if (mode.app.totalCalorieNeed != 0): 
            percent = mode.intakeCalorie*100//mode.app.totalCalorieNeed
        else: 
            percent = '**'
    ...
    def updateIntakeCaloriesEach(mode): 
        caloriesList = []
        for i in range(4): 
            calories = calculateNutrients(0, mode.directoryDict, mode.meals[i], mode.mealsQuant[i])
            caloriesList.append(calories)
        mode.app.breakFastIntakeCalorie = caloriesList[0]
        mode.app.lunchIntakeCalorie = caloriesList[1]
        mode.app.dinnerIntakeCalorie = caloriesList[2]
        mode.app.extraIntakeCalorie = caloriesList[3]
...
class InfoMode(Mode): 
    def appStarted(mode): 
        ...
        mode.initializeEdges()
        mode.startWithSavedData()
        pass 

    def startWithSavedData(mode): 
        my_file = open("/Users/roxena/Documents/CMU Academic Fall 2020/15-112/Mint/termProject/Testing/saveData/test.txt")
        content = my_file.read()
        print(content)
        my_file.close()
        if (content != ''): 
            print('show saved data')
            mode.showSavedDataMode = True 
            # read data 
            infoList = content.split(',')

            mode.name = infoList[0]
            print(mode.name)
            mode.weight = infoList[3]
            mode.gender = infoList[4]

            birthday = infoList[1]
            birthdayList = birthday.split('.')
            mode.bornD = birthdayList[0]
            mode.bornM = birthdayList[1]
            mode.bornY = birthdayList[2]

            height = infoList[2]
            heightlist = height.split('.')
            mode.heightF = heightlist[0]
            mode.heightI = heightlist[1]
        else: 
            mode.showSavedDataMode = False 
    ...
    def saveDataToFile(mode): 
        my_file = open("/Users/roxena/Documents/CMU Academic Fall 2020/15-112/Mint/termProject/Testing/saveData/test.txt", 'w')
        #Roxena Liu,5.5.2001,5.5,130,F
        result = mode.name + ',' + mode.bornD + '.' + mode.bornM + '.' + mode.bornY + ',' + mode.heightF + '.' + mode.heightI + ',' + mode.weight + ',' + mode.gender
    ...
    def drawGender(mode, canvas): 
        ...
        # draw select circle 
        if (mode.chooseFem) or (mode.gender == 'F'): colorF = rgbString(100, 100, 100)
        else: colorF = 'white'
        if (mode.chooseMa) or (mode.gender == 'M'): colorM = rgbString(100, 100, 100)
        else: colorM = 'white'
...
class MyModalApp(ModalApp): 
    def appStarted(app): 
        ...
        [app.breakFastIntakeCalorie, app.lunchIntakeCalorie, app.dinnerIntakeCalorie, app.extraIntakeCalorie] = [0, 0, 0, 0]
        app.suggestedFoods = []
